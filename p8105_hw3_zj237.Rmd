...
head(instacart,5)
```
This dataset consists of `r nrow(instacart)` records, each redord represent an item purchased by a user within an order. `order_id` shows that there are `r nrow(distinct(instacart, order_id))` different orders. the other variables shows the time of the order, the name of the item and their catagories. 
...
And to make a table showing the mean hour of the day, use the folliwing code: 
```{r, message=FALSE}
instacart %>%
...
head(accel_df,5)
```
Thus, the reasulting dataframe `accel_df` consists of $5*7*24*60=50400$ records. Each record contains activity of the male during a minute's period of time, stored in variable `activity`. Other varibles in the record shows information for the one-minute time period. 
The following code creates a tgable for the total activity for each day, and made a plot.
